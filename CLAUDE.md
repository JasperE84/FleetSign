...
- **`web.py` — `create_app` (master) / `create_slave_app` (slave)**: Flask apps served by Waitress. This is the **only** control surface — there are no user-facing console scripts. Single-password session auth (`config.py`), first-run setup, then: the **master** does media CRUD + ordering + per-item duration + scheduling, settings, playback/maintenance, `/status`, uploads, a screens panel, and token-guarded read-only **sync endpoints** (`/sync/manifest`, `/sync/media/<name>`) that slaves pull. The **slave** UI is reduced — local `hwdec`/connection settings, `become-master` — and relies on the master for media and login password. Its waiting page (pre-first-sync) and status page surface the sync connection error so a mis-join is diagnosable from the screen's own UI.
...
- **Exiting maintenance relaunches mpv; it must never re-fullscreen the live one.** *Entering* maintenance un-fullscreens + pauses the running mpv (cheap, keeps the session up), but *exiting* (web "Resume signage" or F12) signals a relaunch (`restart_playback`) instead of sending `set_property fullscreen True`. Re-entering fullscreen recreates mpv's video-output window, and the `loadfile` the player thread fires immediately after lands mid-recreation — a race that hangs mpv on the Pi compositor (dead IPC socket → `BrokenPipeError`, black screen; mpv #3678/#9704). A fresh mpv sequences window-create → first decode internally, like boot. For the same reason `_teardown_mpv` SIGKILLs an mpv that ignores SIGTERM, so a wedged instance can't leave a stale black window up behind the relaunch.
...
- **Joining a slave needs only `master_url` + `sync_token`, never a login password.** The `/setup` join branch ignores the password field; the slave gets its UI password from the master on first sync (`is_configured()` is False until then, so it shows the waiting page). `setup.html` hides the password input in join mode to match — don't re-add a password to the join flow. The waiting/status pages render `sync.last_error` through `friendly_sync_error` (a pure classifier: refused/timeout/bad-token/DNS/data-mismatch) with the raw detail kept underneath; the raw string is master-influenced so it must stay HTML-escaped (Jinja autoescape — never `|safe`). `SyncClient.last_attempt` (set on every attempt, unlike `last_sync` which is last success) gives the never-synced waiting page a timestamp.
